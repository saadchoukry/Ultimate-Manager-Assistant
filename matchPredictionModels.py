...
data = np.load("numpyMatchesV3.npy")
print(data.shape)
...
xTrain, xTest, yTrain, yTest = train_test_split(X, Y,test_size = 0.11, random_state = 2)
...
Building of a deep neural network
    Architecture : 3 hidden layers
        - Input : 3500 examples of 1368 features each
...
def initialize_parameters():
    ...
    W1 = tf.compat.v1.get_variable("W1", [50,1064], initializer = tf.initializers.GlorotUniform())
    ...
    W3 = tf.compat.v1.get_variable("W3", [3, 25], initializer = tf.initializers.GlorotUniform())
...
model(xTrain, yTrain, xTest, yTest, learning_rate = 0.5,
...
clf = LogisticRegression( random_state = 0 , C = 0.001 ).fit(xTrain, yTrain)
